...
parser = argparse.ArgumentParser(description="Run Pollux model for giants.")
parser.add_argument(
    "--n_latents", 
    type=int, 
    required=True, 
    help="Number of latent variables (e.g., 21)"
)
...
output_dir = f"outputs/giants/{n_latents}"
...
plt.savefig(f"{output_dir}/loss_plot.pdf", dpi=300, bbox_inches="tight")
plt.close()
...
plt.savefig(f"{output_dir}/train_labels.pdf", dpi=300, bbox_inches="tight")
plt.close(fig)
...
plt.savefig(f"{output_dir}/test_labels_standardized.pdf", dpi=300, bbox_inches="tight")
plt.close(fig)
...
_ = fig.suptitle(f"Flux only: predicted vs. true labels, giants, no mask, n_latents = {n_latents}", fontsize=22)
plt.savefig(f"{output_dir}/test_labels_unprocessed.pdf", dpi=300, bbox_inches="tight")
plt.close(fig)
...
# save test results
per_star_df.to_csv(f"{output_dir}/test_results_giants.csv", index=False)
